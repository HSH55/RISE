...
from trainer import Qwen2VLGRPOVLLMTrainer, Qwen2VLGRPOTrainer
...
def extract_think_content(text: str) -> str:
    """
    Extract content from the first <think> tag (ensuring only one exists).
    Returns: String content (or None if no tag found).
    """
...
def extract_position_list(answer_str):
    """
    Extract coordinate list in the form [x1, y1, x2, y2] from a string.

    Returns:
        list[int or float] or None
    """
    try:
...
def compute_reward(gt_bbox, pred_bbox, image_size=(1000, 1000), alpha=0.5, beta=0.5, gamma=0):
    """
    gt_bbox / pred_bbox: [x1, y1, x2, y2] or None
    image_size: (width, height) if you want to normalize center distance
    """
...
def safe_append_jsonl_line(file_path, data_list):
    """
    Safely append multiple JSON objects to a .jsonl file, one per line.
...
def extract_all_bboxes(text):
    """
    Extract all coordinate boxes from text, returning multiple [x1, y1, x2, y2] lists.
    """
    try:
        matches = re.findall(r"\[\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*\]", text)
        return [list(map(float, match)) for match in matches]
    except:
        return []
...
def accuracy_reward_iou(completions, solution, **kwargs):
    ...
    for content, sol in zip(contents, solution):
        gt_bboxes = extract_all_bboxes(sol)
        pred_bboxes = extract_all_bboxes(content)

        if not gt_bboxes or not pred_bboxes:
            rewards.append(0.0)
            continue

        reward_matrix = np.zeros((len(gt_bboxes), len(pred_bboxes)))
        for i, gt in enumerate(gt_bboxes):
            for j, pred in enumerate(pred_bboxes):
                reward_matrix[i, j] = compute_reward(gt, pred)

        row_ind, col_ind = linear_sum_assignment(-reward_matrix)
        matched_rewards = [reward_matrix[i, j] for i, j in zip(row_ind, col_ind)]

        avg_reward = sum(matched_rewards) / max(len(gt_bboxes), len(pred_bboxes))
