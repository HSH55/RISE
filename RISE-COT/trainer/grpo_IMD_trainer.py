...
def remove_floats_from_string(text):
    ...
    """
    # 匹配浮点数的正则表达式模式，包括：
    # 1. 标准浮点数 (如 3.14, -0.001, +45.67)
    # 2. 科学计数法 (如 1.23e-4, 5.67E+8)
    # 3. 没有整数部分的小数 (如 .25, -.5)
    # 4. 没有小数部分的浮点数 (如 123., -45.)
...
class Qwen2VLGRPOTrainer_IMD(Trainer):
    """
        ...
        for i, img in enumerate(images):
            ...
            if isinstance(img, Image.Image):
                if img.width == 0 or img.height == 0:
                    print(f"警告: 样本 {i} 的图像尺寸无效 ({img.width}x{img.height})")
                    invalid_indices.append(i)
                    continue
            ...
            # 检查张量
                ...
                if img.numel() == 0:
                    print(f"警告: 样本 {i} 的图像张量为空")
                    invalid_indices.append(i)
                    continue
                if img.dim() != 3 or img.shape[0] not in [1, 3]:
                    print(f"警告: 样本 {i} 的图像张量形状无效: {img.shape}")
                    invalid_indices.append(i)
                    continue
                valid_images.append(img)
                valid_prompts.append(prompts_text[i])
                ...
                print(f"警告: 样本 {i} 的图像类型无效: {type(img)}")
                invalid_indices.append(i)

        # 如果没有有效图像，直接返回空响应
        if not valid_images:
            print("错误: 没有有效的图像数据可用")
            return [""] * len(prompts)

        # 记录无效样本信息
        if invalid_indices:
            print(f"跳过 {len(invalid_indices)} 个无效图像样本: 索引 {invalid_indices}")
    ...
    def _enhance_prompts(self, inputs):
        ...
        for example, response in zip(inputs, generated_responses):
            ...
            new_problem = f"Based on the description: '{description}', provide only the category name of the object in the image. Do not include any additional text."
