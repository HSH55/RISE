...
def newprompt_build(text):
    # Match bounding box - increase fault tolerance
    bbox_pattern = r'bounding box \[(.*?)\]'
    bbox_match = re.search(bbox_pattern, text)
    if not bbox_match:
        return text  # Return original text if match fails
    bounding_box = f"[{bbox_match.group(1)}]"

    # Match content after 'contains' - increase fault tolerance
...
def remove_floats(text):
    # Regular expression pattern to match coordinate format, including possible spaces and newlines
    ...
    # Use re.sub to replace all matched floating point numbers
    cleaned_text = re.sub(float_pattern, '', text)
...
def remove_floats_from_string(text):
    """
    Remove all floating point numbers (including scientific notation) from a string

    Parameters:
        text (str): Input string

    Returns:
        str: String with all floating point numbers removed
    """
    # Regular expression pattern to match floating point numbers, including:
    # 1. Standard floating point numbers (e.g., 3.14, -0.001, +45.67)
    # 2. Scientific notation (e.g., 1.23e-4, 5.67E+8)
    # 3. Decimals without integer part (e.g., .25, -.5)
    # 4. Floating point numbers without decimal part (e.g., 123., -45.)
    pattern = r'[+-]?(\d*\.\d+|\d+\.\d*)([eE][+-]?\d+)?|\d+[eE][+-]?\d+'

    # Remove all matched floating point numbers
    return re.sub(pattern, '', text)


def save_results(completions, cleaned_responses, step_number, save_dir):
    """
    Write completions and cleaned_responses to the same file, saving results prefixed with the current step number.

    Args:
        completions (list): List of model's original responses.
        cleaned_responses (list): List of post-processed clean responses.
...
class Qwen2VLGRPOTrainer_coco(Trainer):
    """
    ...
    def __init__(
        ...
        # Ensure reference model is also moved to GPU
        if self.ref_model is not None:
            print('Reference model also moved to GPU')
    ...
    def _generate_responses(self, inputs):
        ...
        for i, img in enumerate(images):
            if img is None:
                print(f"Warning: Image for sample {i} is None")
            ...
            if isinstance(img, Image.Image):
                if img.width == 0 or img.height == 0:
                    print(f"Warning: Invalid image dimensions for sample {i} ({img.width}x{img.height})")
                    invalid_indices.append(i)
                    continue
                valid_images.append(img)
                valid_prompts.append(prompts_text[i])

            # Check tensor
            elif isinstance(img, torch.Tensor):
                if img.numel() == 0:
                    print(f"Warning: Image tensor for sample {i} is empty")
                    invalid_indices.append(i)
                    continue
                if img.dim() != 3 or img.shape[0] not in [1, 3]:
                    print(f"Warning: Invalid image tensor shape for sample {i}: {img.shape}")
                    invalid_indices.append(i)
                    continue
                valid_images.append(img)
                valid_prompts.append(prompts_text[i])
                ...
                print(f"Warning: Invalid image type for sample {i}: {type(img)}")
                invalid_indices.append(i)

        # If no valid images, return empty responses
        if not valid_images:
            print("Error: No valid image data available")
            return [""] * len(prompts)

        # Log invalid sample information
        if invalid_indices:
            print(f"Skipping {len(invalid_indices)} invalid image samples: indices {invalid_indices}")
