...
class_list = ['fear', 'sadness', 'surprise', 'anger', 'confusion', 'joy', 'love']
def remove_sentences_with_word_and_clean_newlines(text, words):
    """
    Remove sentences containing specific strings from text and clean up extra newlines.

    Parameters:
        text (str): Original text.
        word (str): String to remove.

    Returns:
        str: Text with sentences containing specific strings removed and extra newlines cleaned.
    """
    # First remove all newlines
    text_without_newlines = text.replace('\n', ' ')

    # Use regex to match sentences containing specific strings
    # Assumes sentences end with period, question mark, or exclamation mark
...
def compare_zero3_models(model_a, model_b):
    ...
    # Collect parameters and handle possible None values
    with zero.GatheredParameters(list(model_a.parameters()), modifier_rank=0) as a_params, \
        ...
        # Ensure processing only on rank0 (avoid multi-GPU inconsistency)
        if torch.distributed.get_rank() == 0:
            # Check parameter validity
            if a_params is None or b_params is None:
                print("⚠️ Parameter collection failed, model may not be properly initialized")
                return False
            ...
            return False  # Other ranks return directly
...
def safe_write_tokens(TOKENfile_path, per_token_logps, ref_per_token_logps, per_token_kl):
    ...
    with open(TOKENfile_path, 'a+', encoding='utf-8') as f:
        # Get file lock (prevent multi-process write conflicts)
        fcntl.flock(f, fcntl.LOCK_EX)

        try:
            # Move to end of file (prevent other processes from writing content)
...
def extract_first_class(processed_comp, class_list):
    # Build regex pattern to match all categories in class_list
...
def extract_valid_probabilities(prob_str, class_list):
    """
    Extract all valid key-value pair values from string and combine with class_list to form final answer
    ...
    Returns:
        dict: Final answer dictionary, keys as categories, values as corresponding valid probabilities
    """
    # Build regex pattern
    pattern = r"'(\w+)':(\d+\.?\d*)"  # Match pattern like 'class':value
    ...
    for match in matches:
        ...
        if cls in result:
            try:
                ...
                if 0 <= prob <= 1:  # Check if probability value is between 0 and 1
                    result[cls] = prob
                else:
                    result[cls] = 0  # If probability value not between 0 and 1, set to 0
            except ValueError:
...
def remove_class_names(text, class_list):
    """
    Remove specific class names appearing in text
    """
    # Build regex pattern to match all class names in class_list
    pattern = re.compile(r'\b(' + '|'.join(re.escape(cls) for cls in class_list) + r')\b', flags=re.IGNORECASE)

    # Replace matched class names with empty string
    cleaned = re.sub(pattern, '', text)

    # Post-process residual symbols
    cleaned = re.sub(r'\s{2,}', ' ', cleaned)  # Compress extra spaces
...
def process_text(text):
    ...
    # Remove extra spaces at beginning of paragraphs
    processed_text = re.sub(r'^\s+', '', processed_text, flags=re.MULTILINE)
    # Remove extra spaces at end of paragraphs
    processed_text = re.sub(r'\s+$', '', processed_text, flags=re.MULTILINE)
...
def save_qa_json(questions, completions, step_number, save_dir):
    ...
    Args:
        ...
        step_number (int): Current processing stage number
    ...
    print(f"JSON format results saved to: {file_path}")


class Qwen2VLGRPOTrainer_class_multithink(Trainer):
    """
    ...
    def __init__(
        ...
        # Explicitly specify torch_dtype when loading model
    ...
    def _generate_responses(self, inputs):
        ...
        # Ensure all tensors in prompt_inputs are on GPU
    ...
    def _enhance_prompts(self, inputs):
        """Enhance prompt process"""
        # Batch generate enhanced prompts
        ...
        inputs = [
            ex
            for ex in inputs
            for _ in range(self.num_generations)  # Each input repeated G times
        ]
        ...
        for example, response in zip(inputs, generated_responses):
            ...
            # Build result_str
            result_str = "{" + ",".join(f"{cls}:prob_{i}" for i, cls in enumerate(class_list)) + "}"
            Categories = ', '.join(f"'{cls}'" for cls in class_list)

            # Build new problem description
            new_problem = f"""Analyze the provided image description and generate a probability distribution across emotion categories. Categories: {Categories}.Description: {description}.Output the final answer in the following format: {result_str},where prob_0, prob_1, ..., prob_5 are probabilities summing to 1."""
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Structure processing of generation results
        ...
        # Save with validation (key modification point)
        save_qa_json(
            questions=questions,
