...
def compare_zero3_models(model_a, model_b):
    ...
    with zero.GatheredParameters(list(model_a.parameters()), modifier_rank=0) as a_params, \
        ...
        # 确保只在 rank0 处理（避免多GPU不一致）
        if torch.distributed.get_rank() == 0:
            # 检查参数有效性
            if a_params is None or b_params is None:
                print("⚠️ 参数收集失败，可能模型未正确初始化")
                return False
...
def extract_first_class(processed_comp, class_list):
    # 构建正则表达式模式，匹配 class_list 中的所有类别
...
def extract_valid_probabilities(prob_str, class_list):
    """
    从字符串中提取所有有效的键值对的值，并结合 class_list 形成最终的答案

    参数:
        prob_str (str): 概率分布字符串，格式为 "{class1:prob1,class2:prob2,...}"
        class_list (list): 类别列表，用于确保提取的类别在列表中

    返回:
        dict: 最终的答案字典，键为类别，值为对应的有效概率值
    """
    # 构建正则表达式模式
    pattern = r"'(\w+)':(\d+\.?\d*)"  # 匹配形如 'class':value 的模式
    matches = re.findall(pattern, prob_str)  # 找到所有匹配项
    ...
    for match in matches:
        ...
        if cls in result:
            try:
                prob = float(prob)  # 将概率值转换为浮点数
                if 0 <= prob <= 1:  # 检查概率值是否在0到1之间
                    result[cls] = prob
                else:
                    result[cls] = 0  # 如果概率值不在0到1之间，设为0
            except ValueError:
                # 如果概率值无法转换为浮点数，设置为0
                result[cls] = 0
...
def remove_class_names(text, class_list):
    ...
    移除文本中出现的特定类名
    """
    # 构建正则表达式模式，匹配 class_list 中的所有类名
    pattern = re.compile(r'\b(' + '|'.join(re.escape(cls) for cls in class_list) + r')\b', flags=re.IGNORECASE)
...
def save_qa_json(questions, completions, step_number, save_dir):
    ...
    Args:
        ...
        completions (list): 对应的模型回答列表
        step_number (int): 当前处理阶段编号
        save_dir (str): 文件存储路径
    """
    # 数据校验
    save_dir=save_dir+"/qa_result"
    if len(questions) != len(completions):
        raise ValueError(f"问题数量({len(questions)})与回答数量({len(completions)})不匹配")
    ...
    print(f"JSON格式结果已保存至：{file_path}")


class Qwen2VLGRPOTrainer_class(Trainer):
    """
    ...
    def _generate_responses(self, inputs):
        ...
        generation_config_1.num_return_sequences = 1
    ...
    def _enhance_prompts(self, inputs):
        ...
        for example, response in zip(inputs, generated_responses):
            ...
            # new_problem = f"""According to the following reasoning description of the image:{description}.Output a single danger level for the image from the 'possible_human_no_risk', 'visible_human_no_risk’, 'confirmed_risk'.The output answer format should be as follows:<answer>juedgetype</answer>.Please strictly follow the format."""
            # new_problem = f"""Analyze the image description and classify emotion level: Categories: {class_list}.Description: {description}.Output the final answer in <answer> tags.The output answer format should be as follows:<answer>categorie</answer>.Please strictly follow the format."""
            # 构建 result_str
            ...
            # new_problem = f"""Based on the description: {description}, select one category from {class_list}. Output nothing except the exact category name."""
            ...
            new_example["think"] = f"<think>{response}</think>"

            # 调试输出
            # print("new_example['problem']:")
            print("第二阶段的问题：" * 100)
            print(new_example["problem"])
            print("第二阶段的思维链：" * 100)
            print(new_example["think"])
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # print("input_ids dtype:", prompt_ids.dtype)
        # print("attention_mask dtype:", prompt_mask.dtype)
        ...
        # 访问思考过程
        think = first_example['think']
        question = first_example['problem']
        questions=[question]*self.num_generations

        # if is_conversational(inputs[0]):
        completions = [[{"role": "assistant", "content": think + "<answer>" + completion + "</answer>"}] for completion
                       in completions]
        # 带校验的保存（关键修改点）
        save_qa_json(
            questions=questions,
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                output_reward_func = reward_func(prompts=prompts, completions=completions,step=self.step,outputpath=self.args.output_dir,**reward_kwargs)
